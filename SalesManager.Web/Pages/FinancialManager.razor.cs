...
namespace SalesManager.Web.Pages
{
    public class FinancialManagerBase : ComponentBase
    {
        ...
        protected async Task GetFinancialManagerAsync()
        {
            ...
            balanceGetDTO = await FinancialManagerService.GetBalanceAsync($"FinancialManager/Balance?idUser={Program.GetIdUser()}");
            financialManagersGetDTO = await FinancialManagerService.GetFinancialManagersAsync($"FinancialManager?idUser={Program.GetIdUser()}");
